...
public class MaxHeap   {
    ...
    public void topN(int n){
        ...
        topN.add(0,null);
        n=Math.min(3,maxHeap.size()-1);
        for(int i=1;i<=n;i++){
            topN.add(getMaxValue());
            delete(getMaxValue());
        }
        this.maxHeap=topN;
    ...
    public void insert(Integer value){
        ...
        while (index>1){
            ...
            int parent=maxHeap.get(parentIndex);//获取父节点的值

            if(compare(value,parent)<=0){
                break;
            }else{
                //如果当前的值大于父节点的值，就把父节点的值，赋值给当前节点
                maxHeap.set(index,parent);
...
//        System.out.println(minHeap.getMaxValue());
//
...
//        minHeap.heapSort();
    ...
    public  void  heapSort(){
        int []sorted=new int[maxHeap.size()-1];
        int count=0;
...
//            System.out.println(minHeap);
    ...
    public boolean delete(Integer value){
       ...
       int index =maxHeap.indexOf(value);
       if(index==-1){return  false;}

       int lastIndex=maxHeap.size()-1;

       //得到最后一个元素的值
       Integer temp=maxHeap.get(lastIndex);
       maxHeap.set(index,temp);//用最后一个元素的值，替换被删除的位置

       int parent;

       for(parent=index;parent*2<=maxHeap.size()-1;parent=index){

           //当前节点左孩子的下标
           index=parent*2;
           //如果左节点不是最后一个元素，就说明一定有右子树，同时比较，左右子树，
           if(index!=maxHeap.size()-1 && compare(maxHeap.get(index),maxHeap.get(index+1))<0){
                //如果能够进入这个方法体，说明右孩子比左孩子大，所以index更新
               index=index+1;
           }

           if(compare(temp,maxHeap.get(index))>0){
        ...
        maxHeap.remove(lastIndex);//删除最后一个无用的元素
